...
#define BATCH_SIZE 8
#define USE_FP16
#define SPEED_TEST_W 1280
#define SPEED_TEST_H 864
...
ICudaEngine* PSENet::createEngine(IBuilder* builder, IBuilderConfig* config)
{
    ...
    ITensor* data = network->addInput(input_name_, dt, Dims4{BATCH_SIZE, 3, SPEED_TEST_H, SPEED_TEST_W});
    assert(data);
...
//    config->addOptimizationProfile(profile);
    ...
    config->setMaxWorkspaceSize(1 << 35); // 1G
...
void PSENet::inferenceOnce(IExecutionContext &context, cudaStream_t& stream, void **buffers, float *input, float *output, int input_h, int input_w)
{
    int input_size = BATCH_SIZE * 3 * input_h * input_w * sizeof(float);
    int output_size = BATCH_SIZE * input_h * input_w * 6 / 16 * sizeof(float);

    // DMA input batch data to device, infer on the batch asynchronously, and DMA output back to host
    CHECK(cudaMemcpyAsync(buffers[0], input, input_size, cudaMemcpyHostToDevice, stream));
    context.enqueueV2(buffers, stream, nullptr);
    CHECK(cudaMemcpyAsync(output, buffers[1], output_size, cudaMemcpyDeviceToHost, stream));
...
void PSENet::detect(std::string image_path)
{
    ...
    float* output = new float[BATCH_SIZE * resize_h * resize_w * 6 / 16];
    ...
    mContext->setBindingDimensions(inputIndex, Dims4(BATCH_SIZE, 3, resize_h, resize_w));

    int input_size = BATCH_SIZE * 3 * resize_h * resize_w * sizeof(float);
    int output_size = BATCH_SIZE * resize_h * resize_w * 6 / 16 * sizeof(float);
    ...
    for (int i = 0; i < 1000; ++ i) {
        inferenceOnce(*mContext, stream, buffers, input, output, resize_h, resize_w);
        boxes = postProcess(output, resize_h, resize_w);
    }
    auto end = std::chrono::system_clock::now();
    std::cout << BATCH_SIZE*1000/(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()/1000.0) << "FPS" << std::endl;
    std::cout << (std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()/1000.0)/BATCH_SIZE << "ms" << std::endl;
