...
#define SPEED_TEST_W 960
#define SPEED_TEST_H 640
#define BATCH_SIZE 1
...
//    config->addOptimizationProfile(profile);
    ...
    config->setMaxWorkspaceSize(1 << 34);  // 16MB
#ifdef USE_FP16
...
void doInference(IExecutionContext& context, cudaStream_t& stream, void **buffers, float* input, float* output, int h_scale, int w_scale) {
    ...
    CHECK(cudaMemcpyAsync(buffers[0], input, BATCH_SIZE * 3 * h_scale * w_scale * sizeof(float), cudaMemcpyHostToDevice, stream));
    context.enqueueV2(buffers, stream, nullptr);
    CHECK(cudaMemcpyAsync(output, buffers[1], BATCH_SIZE * h_scale * w_scale * 2 * sizeof(float), cudaMemcpyDeviceToHost, stream));
...
int main(int argc, char** argv) {
    ...
    cv::Mat pr_img = cv::imread("test.jpg");
    ...
    float* data = new float[BATCH_SIZE * 3 * SPEED_TEST_H * SPEED_TEST_W];
    ...
    float* prob = new float[BATCH_SIZE * pr_img.rows * pr_img.cols * 2];
    ...
    context->setBindingDimensions(0, Dims4(BATCH_SIZE, 3, SPEED_TEST_H, SPEED_TEST_W));

    // Create GPU buffers on device
    CHECK(cudaMalloc(&buffers[0], BATCH_SIZE * 3 * SPEED_TEST_H * SPEED_TEST_W * sizeof(float)));
    CHECK(cudaMalloc(&buffers[1], BATCH_SIZE * 2 * SPEED_TEST_H * SPEED_TEST_W * sizeof(float)));
    ...
    for (int i = 0; i < 1000; ++ i) {
        doInference(*context, stream, buffers, data, prob, SPEED_TEST_H, SPEED_TEST_W);
    }
    end = std::chrono::system_clock::now();
    std::cout << BATCH_SIZE*1000/(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()/1000.0) << "FPS" << std::endl;
    float ms_network = (std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()/1000.0)/BATCH_SIZE;
    std::cout << ms_network << "ms" << std::endl;
    ...
    std::cout << 1000/(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()/1000.0) << "FPS" << std::endl;
    float ms_post = (std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()/1000.0);
    std::cout << ms_post << "ms" << std::endl;
    float final_fps = 1000 / (ms_network + ms_post);
    std::cout << "Final FPS: " << final_fps << "FPS" << std::endl;
